...
public class CreateSnapshot extends Dfs {

    public CreateSnapshot(HdfsState state) {
        super(state.subdir);

        Parameter cmd = new CONSTANTSTRINGType("-createSnapshot")
                .generateRandomParameter(state, null);

        Parameter destParameter = new HDFSDirPathType()
                .generateRandomParameter(state, null);
    ...
    public void updateState(State state) {
        ...
        ((HdfsState) state).dfs.createSnapShotFile(dir, filename);
    }
}
